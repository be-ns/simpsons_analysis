...
def load_data():
    ...
    Loads in the two models, the episode DataFrame,
    the scripts, and returns a clear Dataframe with all
    the features in place with the predictions
    on Adaboost and Gradient Boosted.
    '''
    ...
    episode_df = s.return_clean_script_df(
                                        ...
                                        s.clean_episode_data(e_filename)
                                        )
    # fill NaNs
...
def _add_predicts(df, abm, gbm, titles):
    '''
    INPUT: DF with no NaNs, both models for stacked model
    in form `model 1, model 2`, a Pandas DataFrame with
    the title and id of episodes for the TV show.
    (all passed from load_data() function above.)
    OUTPUT: The combination DF of all episodes with all
    features and the predictions for both the first model and the second.
    '''
    # make DF with columns to be returned in suggestion
    df2 = df[[
                'id', 'views', 'imdb_rating',
                'image_url', 'video_url', 'season'
            ]]
    # drop all the columns that aren't helpful
    df.drop(labels=[x for x in df.columns.tolist() if x not in [
                    'id',
                    'number_in_season',
                    'title_len',
                    'election_year',
    ...
    df = df[[
                'id', 'number_in_season', 'title_len',
                'election_year', 'line_lengths',
                'max_line_length', 'major_char_lines',
                'locations_in_ep'
            ]]
