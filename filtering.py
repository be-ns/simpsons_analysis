...
def filter_df(char, scripts = "data/simpsons_script_lines.csv", episodes = "data/simpsons_episodes.csv"):
    ...
    return episode_df.merge(script_df, how='left', left_on = 'id', right_on='episode_id'), scripts

def filter_locations(episode_df, location, scripts):
    '''
    INPUT: DF of episode information, the filepath of the location csv, the filepath of the script csv
...
def get_song(episode_df, scripts, song):
    '''
    INPUT: DF of episode information, filepath for the script csv, a Boolean for filtering on song or on `NOT` song
...
def politicize(episode_df, scripts, politics):
    '''
    INPUT: DF of episode information, filepath for scripts csv, Boolean value for political inclination
    OUTPUT: merged_df (filtered)
    '''
    # read in script csv
    script_df = pd.read_csv(scripts)
    ...
    # get counts for how many times these words appear in the episode / or don't
    ...
    # make sure nothing gets left, but since the score is 0 it means we would skip that episode for the recommender
...
def initialize(char, location, val, song, politics):
    '''
    INPUT: the list of options for the recommender to do some work on - needs a character, a location, a choice between character and location, and two Booleans for musical preference and political preference
    ...
    # start the selection process with whichever value was rated as more important
    if val == 'Characters':
