...
from sklearn.model_selection import RandomizedSearchCV as r_search
...
def clean_episode_data(filename):
    ...
    # election cycle years are all divisible by four
    # (e.g. 2016, 2012, 2008) so I'm adding this as a column feature
    df['election_year'] = df['original_air_date'].apply(
                            lambda x: 1 if x.year % 4 == 0 else 0
                            )

    # drop any unnecesary columns
    df.drop(
        labels=['title', 'imdb_votes', 'production_code'], axis=1, inplace=True
        )
    # return clean df
    return df
...
def return_clean_script_df(fname, df):
    '''
    INPUT: filename as fname and the original episode DataFrame
            from `clean_episode_data`
    ...
    script_df.drop(
        labels=['spoken_words', 'raw_text'], axis=1, inplace=True
        )

    # get totals for each episode
    # for different features and merge into original DF
    line_length_series = script_df.groupby('episode_id')['word_count'].mean()
    monologues = script_df.groupby('episode_id')['word_count'].max()
    df = df.merge(
        pd.DataFrame(line_length_series),
        how='left', left_on='id', right_index=True
        )
    df = df.merge(
        pd.DataFrame(monologues),
        how='left', left_on='id', right_index=True
        )
    # get number of lines spoken by major characters
    # (AKA the Simpsons) and all characters
    major_char_lines = script_df.where(
        script_df['raw_character_text']
        .str.contains('Simpson')
        ).groupby('episode_id')['speaking_line'].count()

    line_series = script_df.groupby(
        'episode_id')['speaking_line'].count()
    # get ratio of lines spoken by Simpsons vs all and merge into DF
    major_char_lines = major_char_lines / line_series
    df = df.merge(
        pd.DataFrame(major_char_lines),
        how='left', left_on='id', right_index=True)

    # get the count of locations from each episode from scripts
    loc_series = add_location_data(script_df)
    df = df.merge(
        pd.DataFrame(loc_series),
        how='left', left_on='id', right_index=True)
...
def build_gbr(
        training_x, training_y, holdout_x, holdout_y, _abr, abr_test, rounds
        ):
    '''
    ...
    while final_test > abr_test:
        ...
        final_model = r_search(
                        _gbr, param_distributions=param_distribution,
        ...
        final_test = predict_on_holdout(
                        _abr, final_model, holdout_x, holdout_y
                        )
...
def build_abr(training_x, training_y, holdout_x, holdout_y, rounds):
    ...
    abr_train = sqrt(abs(np.array(cvs(
                                _abr, training_x, training_y,
                                cv=4, n_jobs=-1, verbose=False,
                                scoring='neg_mean_squared_error'
                                )).mean()))
    ...
    _abr = r_search(
                _abr, param_distributions=param_distribution,
                n_iter=n_iter_search, n_jobs=-1, cv=4, verbose=1
                )
    # fit to training set
...
def stack_models(df, rounds=2):
    '''
    INPUT: clean Pandas Dataframe with script and episode information,
    kwarg for if we are printing RMSE or returning the dataframe itself
    OUTPUT: Either the RMSE for the stacked model or a Dataframe
    with three new columns (scores from each of the three models)
    ...
    # build AdaBoost Regressor
    _abr, abr_test, abr_train = build_abr(
                                    training_x, training_y,
                                    holdout_x, holdout_y, rounds
                                    )
    ...
    # build Gradient Boosted Regressor
    final_model, final_test, final_train = build_gbr(
                                            training_x, training_y,
                                            holdout_x, holdout_y,
                                            _abr, abr_test, rounds
                                            )
    # feature importances gleaned from model not trained
    # with random search due to model limitations
    ...
    # pickle both models with SKLearn's `joblib`
    # (better for complex matrices of data within model)
...
def predict_on_holdout(_abr, random, x_h, y_h):
    ...
    x_h = x_h[[
                'id', 'number_in_season', 'title_len', 'election_year',
                'line_lengths', 'max_line_length',
                'major_char_lines', 'locations_in_ep'
                ]]
...
def plot_scores(df, rmse):
    ...
    df = df.sort_values('id')
    ax.plot(
            df['id'], df['rating'], label='True Rating',
            color='r', lw=5, alpha=.4
            )
    ax.plot(
            df['id'], df['predicted'], label='Predicted Rating',
            color='b', lw=5, alpha=.4
            )
    # adjust preferences and style of figure
    ax.set_xlim((0, 600))
    ax.tick_params(labelsize=20)
    ax.set_title(
                'Predicted Ratings to Actual Ratings, RMSE = {0}'
                .format(round(rmse, 3)), size=40)
...
if __name__ == '__main__':
    ...
    episode_df = return_clean_script_df(
                                        "data/simpsons_script_lines.csv",
                                        clean_episode_data(e_filename)
                                        )
    # get rmse for stacked model with kwarg 'rmse'
